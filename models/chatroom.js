...
export async function createChatroom(isPrivate, name = null) {
  const { rows } = await pool.query(
    `
    INSERT INTO chatroom(is_private, name) values('${isPrivate}', '${name}') RETURNING id;
    `,
  );
  return rows[0].id;
}

export async function createChatroomUser(chatroomId, userIds, connection = pool) {
